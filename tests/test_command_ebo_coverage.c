...
** Zappy
** File description:
** test_command_ebo_coverage - Simple coverage tests for command_ebo functions
*/
...
extern void __real_send_ebo_command(server_t *server, int egg_id);

// Simple test server creation with graphical clients
static server_t *create_simple_ebo_server(void)
{
...
static void free_simple_ebo_server(server_t *server)
{
    if (!server) return;
...
TestSuite(command_ebo_coverage_tests);

// Test: send_ebo_command with NULL server
Test(command_ebo_coverage_tests, test_send_ebo_command_null_server)
{
    __real_send_ebo_command(NULL, 123);
    
    cr_assert(true, "Function should handle NULL server");
}

// Test: send_ebo_command with no graphical clients
Test(command_ebo_coverage_tests, test_send_ebo_command_no_graphics)
{
    server_t *server = create_simple_ebo_server();
    ...
    __real_send_ebo_command(server, 456);
    
    cr_assert(true, "Function should handle no graphical clients");
    
    free_simple_ebo_server(server);
}

// Test: send_ebo_command with valid parameters
Test(command_ebo_coverage_tests, test_send_ebo_command_valid)
{
    server_t *server = create_simple_ebo_server();
    
    __real_send_ebo_command(server, 789);
...
Test(command_ebo_coverage_tests, test_send_ebo_command_different_ids)
{
    server_t *server = create_simple_ebo_server();
    
    __real_send_ebo_command(server, 0);
    __real_send_ebo_command(server, 999);
    __real_send_ebo_command(server, -1);
...
// Test: send_ebo_command with multiple graphical clients
Test(command_ebo_coverage_tests, test_send_ebo_command_multiple_clients)
{
    server_t *server = create_simple_ebo_server();
    ...
    __real_send_ebo_command(server, 555);
...
// Test: send_ebo_command execution path coverage
Test(command_ebo_coverage_tests, test_send_ebo_command_execution_path)
{
    server_t *server = create_simple_ebo_server();
    
    // This will test the buffer allocation and formatting logic
    __real_send_ebo_command(server, 12345);
