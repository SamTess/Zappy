...
static char *format_pdr_response(int client_id, resource_type_t resource_type)
{
    char *buffer = NULL;
    int size = 0;
    ...
    if (!buffer)
        return NULL;
    snprintf(buffer, size + 1, "pdr #%d %d\n", client_id, resource_type);
    return buffer;
}
    ...
    zappy_client_t *clients, resource_type_t resource_type)
{
    ...
    if (!client || !client->player || !client->client || !clients)
        return;
    buffer = format_pdr_response(client->client->client_id, resource_type);
    if (!buffer)
        return;
    while (current) {
